import type { ColorBuffer, ColorMatrix } from '../core/types';
import { createBuffer, createMatrix } from '../core/shared';
...
export function multiplyMatrixVector(
  ...
  for (let i = 0; i < 3; i++) {
    const row = matrix[i];
    output[i] = row[0] * v0 + row[1] * v1 + row[2] * v2;
  }
}
...
const lmsD65 = createBuffer(3);
const lmsD50 = createBuffer(3);
...
const lmsScratch = createBuffer(3);
