import type { ColorMode, ColorState } from './types';
import { createColor } from './shared';

export interface ParsedColor extends ColorState {
  alpha: number;
}
...
export function parseColor(css: string): ParsedColor {
  ...
  if (trimmed[0] === '#') {
    ...
    return {
      ...createColor([r / 255, g / 255, b / 255], 'rgb'),
      alpha: a,
    };
  }
  ...
  if (openParen !== -1 && closeParen !== -1) {
    ...
    const state = createColor([v0, v1, v2], mode);
