...
describe('D50 Adapter (XYZ <-> Lab)', () => {
  ...
  describe('labToXyz50', () => {
    it('should cover the linear slope branch for very dark colors (L < 8)', () => {
      const darkLab = createBuffer([1.0, 0, 0]);
      const xyz = createBuffer([0, 0, 0]);

      labToXyz50(darkLab, xyz);

      expect(xyz[1]).toBeGreaterThan(0);
      expect(xyz[1]).toBeLessThan(0.01);

      const backToLab = createBuffer([0, 0, 0]);
      xyz50ToLab(xyz, backToLab);
      expect(backToLab[0]).toBeCloseTo(1.0, 4);
    });
  });
